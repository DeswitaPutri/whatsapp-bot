...
const getPointId = (userId, _dir) => {
    ...
    if (found === false && pos === null) {
        ...
        _dir.push(obj)
        fs.writeFileSync('./datagame/point.json', JSON.stringify(_dir))
        return userId
    } else {
        return _dir[pos].id
    }
...
const getLevelingPoint = (userId, _dir) => {
    ...
    if (found === false && pos === null) {
        const obj = { id: userId, point: 5, level: 1 }
        _dir.push(obj)
        fs.writeFileSync('./datagame/point.json', JSON.stringify(_dir))
        return 5
    } else {
...
const reduceLevelingLevel = (userId, amount, _dir) => {
    let position = null
    Object.keys(_dir).forEach((i) => {
        if (_dir[i].id === userId) {
            position = i
...
const reduceLevelingPoint = (userId, amount, _dir) => {
    let position = null
    Object.keys(_dir).forEach((i) => {
        if (_dir[i].id === userId) {
            position = i

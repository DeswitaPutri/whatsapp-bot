...
function normalizeMention(from, txt, mention=[]){
    if(! mention.length > 0){
...
//send Video
exports.sendVideo = function(to, bufer, text=""){
...
exports.getParticipantIds = async function(to) {
    var yo = await wa.groupMetadata(to)
    var members = yo.participants
    let mids = []
...
exports.ReplyFakeStatus = async function(deswita,to, text, prevtext, msgId="B826873620DD5947E683E3ABE663F263"){
    waid = "0@s.whatsapp.net"
    deswita.sendMessage(to, text, MessageType.text, {contextInfo: {'stanzaId': msgId, 'participant': waid, 'remoteJid': 'status@broadcast', 'quotedMessage': {"imageMessage": {"caption": prevtext, 'jpegThumbnail': fs.readFileSync('./media/deswita.jpeg')}}}})
}
...
exports.setThumbnail = async function(to, url, title, desc, filename){
    ...
    var yo = await wa.generateLinkPreview(url)
    yo.title = title;
    yo.description = desc;
    yo.jpegThumbnail = bufer;
    wa.sendMessage(to, yo, MessageType.extendedText, anu)
}
